...
The document's standard, stated in its "How claims are made" front matter:
  ...
  * the measured refit floor is printed beside it as a magnitude reference, never as a
...
separately.
    ...
    from effects import effect, REFIT_FLOOR
...
# (`refit_floor.py`, results/refit_floor.csv). Printed beside every effect so a reader can
# tell a large difference from a small one.
REFIT_FLOOR: dict[str, dict[str, float]] = {
...
class Effect:
    ...
    def equivalent_within(self, margin: float | None = None) -> bool:
        """True when the interval lies entirely inside +/- margin (default: the floor).
        ...
        """
        m = self.floor if margin is None else margin
    ...
    def __str__(self) -> str:
        ...
        rel = ""
        if self.floor:
            rel = (f", refit floor {self.floor:.4g}"
                   f"{' — below it' if abs(self.delta) < self.floor else ''}")
