...
- **Execution ordering and issue sizing.** How the cleanups carve into issues is ticket 11's
  call. **Partly settled:** ticket 10 fixes the one ordering constraint — its scripts/root sweep
  lands first — and the running budget is ~14-15 against ticket 06's ~15 tripwire, so ticket 13
  has roughly one issue of headroom.
