...
- [Rule on `scripts/` and the tracked root clutter](issues/10-scripts-and-root-clutter.md) — **both plot scripts deleted, not fixed**, which **resolves ticket 12's conditional**: `forecast_from_checkpoint` and `holdout_actuals_NT` die with them, so the `evaluation/` reshape is the three-file shape with no pending branch. The ticket proposed fixing one since "each holds the other's fix"; against that, their job is `plot_suite_forecast`'s (**16 notebook calls**), the covariate comparison is live but done in the notebooks via `removable_features` (**16 occurrences**), and **neither script has ever produced a tracked output** — both write to the gitignored `Predictions/`, and `figures/` holds no covariate figure at all. Also deleted: the three one-shots (409 lines; the `ParetoNBD_MLE` provenance fact relocates to `archive/README.md`), `inputs_configs/` (zero references anywhere) and `PUBLISHING.md` (documents a path the map lists as out of scope). **Kept:** `trace_golden_reachability.py` — *being documented* (`docs/running-a-model.md:206`) is what separates a tool from a leftover, and it is the check that ticket 11 orphaned nothing; `figures/`, `archive/`, `VastAI/` (+1 README line); and `.agents/` **stays vendored**, with the counter-argument recorded for the hand-to-examiner moment rather than dismissed. **Three inventory corrections:** `instruction` and `Package_Notebook.code-workspace` were **already gitignored and untracked**, so two of the eight clutter items were moot; `.agents/` is **74 tracked files / 608 KB — more than `src/` (32)**, the largest tracked non-package block in the repo, given one bullet among eight; and `.Rhistory` needs a gitignore rule it never had. **Two rulings that bind other tickets. Budget: this ticket folds to +0**, leaving the running total at ~14-15 — so **ticket 06's tripwire now binds on ticket 13**, which inherits roughly one issue of headroom and should therefore widen the net around *one* surface, most likely audit 03's silently-failing Transformer rollout. **Order: this sweep lands FIRST**, before any structural issue — the deleted scripts import nine names across `models/`, `evaluation/` and `benchmarks/`, three subpackages ticket 11 reshapes and ticket 09 renames, so landing it last would force every issue in between to keep two broken scripts compiling against moving targets. This is the map's one ordering constraint.

- [How wide the safety net needs to be](issues/13-safety-net-scope.md) — **the net widens once, around the rollout seam, at +1 issue — running total ~15, at the tripwire and not over it.** The standalone issue **parametrises `tests/test_golden_end_to_end.py` over model family** and lands **FIRST, ahead of ticket 10's sweep** (whose constraint restates as "first among structural issues"), because a net added after the change pins whatever the change produced. Four arms: `transformer` and `valendin_lstm` pin metrics + determinism like the existing `lstm` arm; `pareto_nbd` gets **shape/finiteness/determinism only, no pinned values** — it never trains or rolls out, and its 200-draw single-chain MCMC would pin noise. Second deliverable, and why it is one session: `trace_golden_reachability.py` imports all four scenarios **from the test** and its own `_train_and_roll` copy of the pipeline is deleted — a duplication removed, not code added. **Delivery rule: a "the number did not change" test lands before the change and stands alone; a "the structure is wired up" test folds into the issue that creates it at +0.** Three fold in: the registry rewrite of `test_model_registration.py` (keeping its per-type class assertions, which guard a dispatch-fallthrough risk the registry does not remove), retired-name assertions on `test_imports.py:91`'s existing pattern, and a ~10-line **subpackage-acyclicity** test riding decision 6 — not a re-opening of ticket 08, which dropped a torch-*cost* proxy, where this asserts the import graph. **Ticket 07's `to_rollout()` issue may NOT re-baseline the golden numbers**: the test forecasts from the checkpoint today, so 07's `fit_model` defect is invisible until `to_rollout()` reads the in-memory object — the unchanged numbers at rel=1e-6 ARE the proof the `best_state` load-back landed. **`run_study_suite` smoke test DECLINED with reasons recorded** (floor rescinded, loud failure mode, refactor changes its dispatch not its writes, most expensive candidate) alongside five other accepted gaps. **Four findings: there is no CI in this repo at all** — nothing runs pytest or the tracer automatically, so every "runs" in this map means someone ran it; the Transformer rollout is **already exercised and unasserted** by the tracer, so audit 03's gap is a missing assertion rather than missing code; the tracer holds a **second implementation of the golden pipeline** the ledger missed by scanning only `src/`; and the four scenarios are not one shape.

- [Cut the execution issue set](issues/11-cut-execution-issue-set.md) — **15 issues published to `.scratch/package-cleanup/issues/`** with a spec at `spec.md`; the map's destination is reached and it writes no production code. Ordering holds as charted: **01 the parametrised golden test, first overall**; **02 the scripts/root sweep, first among structural**; **15 the orphan rename sweep, last by construction**. **Two accounting findings:** the map's budget had a **one-issue gap** — ticket 09 folded the `trials/` creation into "the issue that creates `trials/`", which ticket 06's ten issues never contained, so the honest carve was **16**; and **the designated deferral would not have freed a slot**, because ticket 12's `analysis.py` issue also carries the group collapse and the single Student-t interval, both wrong-number fixes that must land. Resolved by **merging refit-only with the `trials/` creation** (issue 05) — same module, neither moves a number — landing at **15, at the tripwire and not over**. **Three ownership gaps filled:** every `CLAUDE.md` edit has a named owner as ticket 07 demanded (Edits 1-3 → issue 06, Edit 4 plus the heading removal → issue 07, the `scripts/` line → issue 02); **ticket 08's torch-free removal was unowned** — six sites inventoried, no issue assigned — now split across issues 06 and 09 with the two unrelated lazy imports named as not-to-be-swept; and the Pareto grid file rename moved from the sweep into issue 10, which splits that module.
...
- **Execution ordering and issue sizing.** How the cleanups carve into issues is ticket 11's
  call. **Nothing else is left to settle for it:** ticket 13 spent its one issue of headroom, so
  the running budget is **~15 against ticket 06's ~15 tripwire** — at it, not over it, and no cut
  is required. Two ordering constraints, in this order: **ticket 13's net lands first**, then
  **ticket 10's scripts/root sweep, first among the structural issues**. If ticket 11's own carve
  lands at 16, the deferral is ticket 12's nominee — the `studies/analysis.py` three-way split.
